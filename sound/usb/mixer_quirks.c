 ...
 * by controlling the volume on each possible crossing point.
 * Volume control is linear, from -inf (dec. 0) to +6dB (dec. 65536) with
 * 0dB being at dec. 32768. This device is very similar to the RME Babyface Pro
 */
enum {
	SND_BBF_CTL_REG1 = 0,
	SND_BBF_CTL_REG2
...
#define SND_BBF_MIXER_VAL_SHIFT 9
#define SND_BBF_MIXER_VAL_MIN 0 // -inf
#define SND_BBF_MIXER_VAL_MAX 65536 // +6dB

#define SND_BBF_USBREQ_CTL_REG1 0x10
#define SND_BBF_USBREQ_CTL_REG2 0x17
#define SND_BBF_USBREQ_MIXER 0x12

static int snd_bbf_ctl_update(struct usb_mixer_interface *mixer, u8 reg,
	u8 index, u8 value)
{
	int err;
	u16 usb_req, usb_idx, usb_val;
	...
	if (reg == SND_BBF_CTL_REG1) {
		usb_req = SND_BBF_USBREQ_CTL_REG1;
		if (index == SND_BBF_CTL_REG1_CLK_OPTICAL) {
			usb_idx = 3;
			usb_val = value ? 3 : 0;
		} else {
			usb_idx = 1 << index;
		...
		usb_req = SND_BBF_USBREQ_CTL_REG2;
		usb_idx = 1 << index;
		usb_val = value ? usb_idx : 0;
	}
...
static int snd_bbf_ctl_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	u8 reg, idx, val;
	int pv;
...
static int snd_bbf_ctl_info(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
	u8 reg, idx;
	int pv;
...
static int snd_bbf_ctl_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int err;
	u8 reg, idx;
	...
	kcontrol->private_value = reg
		| ((idx & SND_BBF_CTL_IDX_MASK) << SND_BBF_CTL_IDX_SHIFT)
		| ((val & SND_BBF_CTL_VAL_MASK) << SND_BBF_CTL_VAL_SHIFT);

	err = snd_bbf_ctl_update(mixer, reg, idx, val);
	return err < 0 ? err : 1;
}

static int snd_bbf_ctl_resume(struct usb_mixer_elem_list *list)
{
	u8 reg, idx;
	int value, pv;
	...
	reg = pv & SND_BBF_CTL_REG_MASK;
	idx = (pv >> SND_BBF_CTL_IDX_SHIFT) & SND_BBF_CTL_IDX_MASK;
	value = (pv >> SND_BBF_CTL_VAL_SHIFT) & SND_BBF_CTL_VAL_MASK;

	return snd_bbf_ctl_update(list->mixer, reg, idx, value);
}

static int snd_bbf_vol_update(struct usb_mixer_interface *mixer, u16 index,
	u32 value)
{
	struct snd_usb_audio *chip = mixer->chip;
	int err;
	...
	idx = index & SND_BBF_MIXER_IDX_MASK;
	// 18 bit linear volume, split so 2 bits end up in index.
	v = value & SND_BBF_MIXER_VAL_MASK;
	usb_idx = idx | (v & 0x3) << 14;
	usb_val = (v >> 2) & 0xffff;

	err = snd_usb_ctl_msg(chip->dev,
			      usb_sndctrlpipe(chip->dev, 0),
			      SND_BBF_USBREQ_MIXER,
			      USB_DIR_OUT | USB_TYPE_VENDOR |
			      USB_RECIP_DEVICE,
			      usb_val, usb_idx, NULL, 0);
...
static int snd_bbf_vol_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] =
		kcontrol->private_value >> SND_BBF_MIXER_VAL_SHIFT;
	return 0;
}

static int snd_bbf_vol_info(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
	uinfo->value.integer.min = SND_BBF_MIXER_VAL_MIN;
	uinfo->value.integer.max = SND_BBF_MIXER_VAL_MAX;
	return 0;
}

static int snd_bbf_vol_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int err;
	u16 idx;
	...
	uvalue = ucontrol->value.integer.value[0];
	idx = kcontrol->private_value & SND_BBF_MIXER_IDX_MASK;
	old_value = kcontrol->private_value >> SND_BBF_MIXER_VAL_SHIFT;

	if (uvalue > SND_BBF_MIXER_VAL_MAX)
		return -EINVAL;
...
static int snd_bbf_vol_resume(struct usb_mixer_elem_list *list)
{
	...
	return snd_bbf_vol_update(list->mixer, idx, val);
}

// Predefine elements
static const struct snd_kcontrol_new snd_bbf_ctl_control = {
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	.access = SNDRV_CTL_ELEM_ACCESS_READWRITE,
	.index = 0,
	.info = snd_bbf_ctl_info,
	.get = snd_bbf_ctl_get,
	.put = snd_bbf_ctl_put
};

static const struct snd_kcontrol_new snd_bbf_vol_control = {
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	.access = SNDRV_CTL_ELEM_ACCESS_READWRITE,
	.index = 0,
	.info = snd_bbf_vol_info,
	.get = snd_bbf_vol_get,
	.put = snd_bbf_vol_put
};

static int snd_bbf_ctl_add(struct usb_mixer_interface *mixer, u8 reg,
	u8 index, char *name)
{
	struct snd_kcontrol_new knew = snd_bbf_ctl_control;

	knew.name = name;
	knew.private_value = (reg & SND_BBF_CTL_REG_MASK)
		| ((index & SND_BBF_CTL_IDX_MASK) << SND_BBF_CTL_IDX_SHIFT);

	return add_single_ctl_with_resume(mixer, 0, snd_bbf_ctl_resume,
					  &knew, NULL);
}

static int snd_bbf_vol_add(struct usb_mixer_interface *mixer, u16 index,
	char *name)
{
	struct snd_kcontrol_new knew = snd_bbf_vol_control;

	knew.name = name;
	knew.private_value = index & SND_BBF_MIXER_IDX_MASK;

	return add_single_ctl_with_resume(mixer, 0, snd_bbf_vol_resume, &knew,
					  NULL);
}
...
int snd_usb_mixer_apply_create_quirk(struct usb_mixer_interface *mixer)
{
	...
	case USB_ID(0x2a39, 0x3fb0): /* RME Babyface Pro FS */
		...
		break;
	case USB_ID(0x0424, 0x3fb7): /* RME Babyface */
		err = snd_bbf_controls_create(mixer);
		break;
